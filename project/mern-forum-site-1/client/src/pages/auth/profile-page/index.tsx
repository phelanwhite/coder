...
import ProfileInformationForm from "@/components/commons/auth/ProfileInformationForm";
  ...
  return (
    <>
      ...
      <div className="flex flex-col-reverse iPad:flex-row max-iPad:gap-6 items-start justify-evenly max-w-[1336px] mx-auto">
        {/* right  */}
        <div className="max-w-[728px] w-full space-y-10 px-6">
          {/* title  */}
          ...
          <div className="overflow-x-auto border-b border-borderColor pb-4 ">
            <div className="flex items-center gap-6 text-textSecondaryColor">
              {authLinks.profile.list.map((link) => (
                <Link
                  key={link.title}
                  to={authLinks.profile.path + link.path}
                  className={clsx(
        ...
        {/* left  */}
        <div className="max-w-[350px] w-full iPad:border-l border-[#F2F2F2] px-6 space-y-10">
