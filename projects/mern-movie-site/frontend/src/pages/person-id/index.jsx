...
import { person_movie_type } from "features/person/services/containts";
import { useAddPersonMutation } from "features/person/services/myPersonApi";
...
const PersonId = () => {
  const { id } = useParams();
  const [addPseron, addPseronResult] = useAddPersonMutation();
  useEffect(() => {
    if (addPseronResult.isSuccess) {
      toast.success(addPseronResult.data?.message);
    }
    if (addPseronResult.isError) {
      toast.error(addPseronResult.error.data?.message);
    }
  }, [addPseronResult]);
