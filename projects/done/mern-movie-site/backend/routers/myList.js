...
myListRouter.put(`/:id`, verifyToken, async (req, res, next) => {
  try {
    ...
    // create new myList
    const newFavourite = await myListModel.findByIdAndUpdate(id, body, {
      new: true,
    });
...
myListRouter.get(`/check-exists`, verifyToken, async (req, res, next) => {
  try {
    const user = req.user;
    const { id, type } = req.query;

    const existing = await myListModel.findOne({
      user: user._id,
      items: {
        $elemMatch: {
          id,
  ...
  try {
    ...
    if (existingItemInMyList) {
      return res.status(200).json({
        ...
        data: existingItemInMyList,
      });
    }

    // add item in my list
    const newItem = await myListModel.findByIdAndUpdate(
      id,
      {
        $push: {
          items: {
