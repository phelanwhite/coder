...
myFavouriteRouter.post(`/`, verifyToken, async (req, res, next) => {
  try {
    ...
    // check
    const existingFavourite = await myFavouriteModel.findOne({
      user: user._id,
      favouriteId: body.favouriteId,
      favouriteType: body.favouriteType,
    });
    ...
    const newFavourite = await myFavouriteModel.create({
      user: user?._id,
      ...body,
    });
    return res.status(201).json({
...
myFavouriteRouter.get(`/check-exists`, verifyToken, async (req, res, next) => {
  try {
    const user = req.user;
    const { favouriteId, favouriteType } = req.query;
    const existingFavourite = await myFavouriteModel.findOne({
      user: user._id,
      favouriteId,
      favouriteType,
